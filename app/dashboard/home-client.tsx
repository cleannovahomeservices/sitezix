...
import { Plus, Send, Loader2, Layers, User, ShoppingCart, FileText, BarChart3, Globe, Zap, Code, Copy, Download, ExternalLink, Sparkles } from 'lucide-react';
...
export function HomeClient({ userName, credits: initialCredits }: { userName: string; credits: number }) {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setResult({ projectId: json.projectId, html: json.html });
      setCredits((c) => Math.max(c - 1, 0));
      toast('Site generated!');

      // Auto-deploy if pill is on
      if (pills.deploy) {
        autoDeploy(json.projectId);
      }
    } catch (e: unknown) {
